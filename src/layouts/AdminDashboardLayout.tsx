...
import { Home, Calendar, Users, Briefcase, BookOpen, UserCheck, ImageIcon, FileText, BarChart, Settings, Bell, Search, Shield, BellRing, User as UserIcon, LogOut, Sparkles, ArrowRight } from 'lucide-react';
...
export default function AdminDashboardLayout() {
  ...
  const navGroups = [
    ...
    },
    {
      title: 'People',
      items: [
        { name: 'Coordinators', path: '/admin/coordinators', icon: UserCheck },
  ...
  return (
    <div className="bg-[#EFEFEF] min-h-screen flex relative overflow-hidden font-sans text-gray-900">
      {/* Full-Screen Animated WebGL Shader Background Overlay */}
      ...
      <aside className="w-64 bg-white/85 backdrop-blur-xl text-gray-800 fixed h-full z-20 flex flex-col hidden md:flex border-r border-white/60 shadow-lg">
        ...
        <div className="flex-1 overflow-y-auto py-5 px-4 custom-scrollbar space-y-6">
          {navGroups.map((group, idx) => (
            <div key={idx}>
              <h3 className="px-3 text-[10px] font-bold text-gray-600 uppercase tracking-wider mb-2">
                {group.title}
              </h3>
      ...
      <div className="flex-1 md:ml-64 flex flex-col relative z-10 min-h-screen">
        ...
        <header className="h-16 bg-white/80 backdrop-blur-xl border-b border-white/60 sticky top-0 z-20 flex items-center justify-between px-4 sm:px-6 lg:px-8 shadow-xs">
          ...
          <div className="flex-1 flex items-center">
            <div className="relative w-full max-w-md hidden sm:block">
              ...
              <input
                type="text"
                placeholder="Search campus events, reports, departments..."
          ...
          <div className="flex items-center space-x-3">
            <button 
              ...
              className="p-2 text-gray-600 hover:text-[#F26522] hover:bg-white/80 rounded-full transition-colors cursor-pointer"
              title="View Calendar"
            >
                ...
                <div className="absolute right-0 mt-2 w-80 bg-white/95 backdrop-blur-xl rounded-2xl shadow-xl border border-gray-200/80 overflow-hidden z-50 animate-in zoom-in-95 duration-200">
                  ...
                  <div className="divide-y divide-gray-100 max-h-80 overflow-y-auto text-xs">
                    <div className="p-3.5 hover:bg-gray-50 cursor-pointer">
                      <div className="font-bold text-gray-900 mb-0.5">SIH Hackathon Proposal Submitted</div>
                      <div className="text-[11px] text-gray-600 line-clamp-2">CSE Department submitted NBA/NAAC event report draft for review.</div>
                      <div className="text-[10px] text-[#F26522] font-semibold mt-1">10 mins ago</div>
                    </div>
                    <div className="p-3.5 hover:bg-gray-50 cursor-pointer">
                      <div className="font-bold text-gray-900 mb-0.5">1st Convocation Report Archived</div>
                      <div className="text-[11px] text-gray-600 line-clamp-2">Degree awards & gold medal event report filed into central university repository.</div>
                      <div className="text-[10px] text-gray-400 font-semibold mt-1">1 hour ago</div>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="relative border-l border-gray-200/80 pl-3">
              ...
              >
                <div className="text-right hidden sm:block">
                  <div className="text-xs font-bold text-gray-900 group-hover:text-[#F26522] transition-colors">{profile?.full_name || 'Dr. Faculty Admin'}</div>
                  <div className="text-[10px] font-semibold text-gray-500 uppercase">{profile?.role || 'FACULTY ADMIN'}</div>
                </div>
              ...
              {showProfileMenu && (
                <div className="absolute right-0 mt-3 w-56 bg-white/95 backdrop-blur-xl rounded-2xl shadow-xl border border-gray-200/80 overflow-hidden z-50 animate-in zoom-in-95 duration-200">
                  <div className="px-4 py-3 border-b border-gray-100 bg-gray-50/50 sm:hidden">
                    <div className="text-xs font-bold text-gray-900">{profile?.full_name || 'Dr. Faculty Admin'}</div>
                    <div className="text-[10px] font-semibold text-gray-500 uppercase">{profile?.role || 'FACULTY ADMIN'}</div>
                  </div>
        ...
        {/* PAGE CONTENT CONTAINER WITH LIQUID GLASS CARDS */}
